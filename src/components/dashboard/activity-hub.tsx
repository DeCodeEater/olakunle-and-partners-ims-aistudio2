import { ArrowRightLeft, Verified, FileText, Check } from 'lucide-react';

const activities = [
  {
    title: 'New Lease Signed - Unit 4B',
    subtitle: 'Victoria Island Plaza • 2h ago',
    type: 'LEASE',
  ...
  {
    title: 'Commission Payout Pending',
  ...
  {
    title: 'New Identity Document Uploaded',
    subtitle: 'Lekki Commercial Hub • 2d ago',
    type: 'DOC',
    typeColor: 'bg-slate-500/20 text-slate-300 border border-slate-500/30',
    indicatorColor: 'bg-slate-400',
    urgency: 'informational'
  }
];
    ...
    <div className="bg-deep-slate text-pure-surface rounded-lg p-fluid-s lg:p-fluid-m flex flex-col md:flex-row gap-fluid-m shadow-sm h-full">
      <div className="md:w-[35%] flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-center mb-fluid-s">
            <div>
              <h3 className="font-headline text-fluid-lg font-medium w-full truncate">Transactions Hub</h3>
              <p className="font-mono text-fluid-xs text-surface-dim/70 mt-1 font-medium">Recent Actions</p>
            </div>
        ...
        <button className="w-full min-h-[44px] bg-pure-surface/5 hover:bg-pure-surface/10 border border-pure-surface/10 text-center text-[10px] font-mono font-bold text-pure-surface uppercase tracking-widest transition-all duration-300 py-3 rounded group mt-4">
      ...
      <div className="md:w-[65%] flex flex-col md:border-l border-t md:border-t-0 border-pure-surface/10 pt-6 md:pt-0 md:pl-fluid-m">
        <div className="flex justify-between items-end border-b border-pure-surface/10 pb-2 mb-2">
          <h4 className="font-mono text-[11px] font-bold text-pure-surface flex items-center">
        ...
        <div className="space-y-1">
          {activities.map((act, i) => (
            <div key={i} className="flex justify-between items-center group cursor-pointer py-3 min-h-[44px] border-b border-pure-surface/5 last:border-0 hover:bg-pure-surface/5 px-2 -mx-2 rounded transition-colors relative">
              <div className="flex items-center gap-3">
                <div className={`w-1.5 h-1.5 rounded-full ${act.indicatorColor} group-hover:scale-125 transition-transform`} title={act.urgency === 'action' ? 'Requires action' : 'Informational'}></div>
                <div>
                  <p className={`font-body text-fluid-sm font-medium opacity-90 ${act.urgency === 'action' ? 'text-pure-surface font-semibold' : 'text-slate-300'}`}>{act.title}</p>
              ...
              <div className="flex items-center gap-2">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                  <button className="text-[9px] flex items-center gap-1 font-mono text-slate-400 hover:text-white uppercase" title="Mark as read">
